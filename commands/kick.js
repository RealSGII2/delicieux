function kickMember(bot, message, member, reason) {
	if (member.highestRole.position >= message.member.highestRole.position) return message.reply("You do not have permission to kick this member!").catch(() => bot.safeSend(message, module.exports.help.name));
	if (!member.kickable) return message.reply("I do not have permission to kick this member!").catch(() => bot.safeSend(message, module.exports.help.name));
